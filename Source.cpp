...
double problem_1(int N) {
	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			E[i][j] = (double)rand() / RAND_MAX * (15 - 2) + 5;
			//cout << E[i][j] << " ";
		}
		//cout << "\n";
	}
...
#pragma omp parallel for shared(M1, M2, M3) private(i,j,k)
	...
	printf("Время выполнения: %.4f second(s)\n", ((double)end - start) / ((double)CLOCKS_PER_SEC));
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			//printf("%f\t", M3[i][j]);
		}
...
int main()
{
	...
	printf("Ускорение: %.4f\n", time1 / time2);
	printf("Эффективность: %.4f\n", (time1 / time2) / num_threads);

	/*num_threads = 1;
	time2 = problem_2(N, num_threads);
	boost = time1 / time2;
	printf("Ускорение: %.4f\n", time1 / time2);
	printf("Эффективность: %.4f", (time1 / time2) / num_threads);*/
}